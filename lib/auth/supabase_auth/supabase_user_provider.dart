...
class WatamealSupabaseUser extends BaseAuthUser {
  WatamealSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> watamealSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = WatamealSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
